...
**本周的特殊性:** TSP 是计算复杂度理论里最经典的 NP-hard 问题——你**不能**像第 7 周那样指望 LP 给你整数解(没有 TU)。它需要全套 IP 武器:
1. **公式建模**——和前几周类似的 IP。
2. **子环路消除约束 (SEC)**——本周新概念,数量是指数级,无法显式列出。
3. **懒约束生成**——只在需要时加约束,把指数级的问题压缩到秒级求解。

这种"延迟添加约束"的思想是**现代 IP 实务**的核心——许多大规模优化问题都用它。

## 1. 为什么 TSP 很难

对称情况下,不同行程的数量 = $(n-1)!/2$。当 $n = 60$ 时:$\approx 10^{81}$。宇宙的年龄只有 $10^{10}$ 年。暴力法不可能。

**对比直觉**:即使每秒能枚举 $10^{12}$ 条行程(超级计算机水平),60 个城市要跑 $10^{69}$ 秒——远超宇宙寿命。所以暴力枚举完全不现实,**必须**用聪明的 IP 方法。

而 Gurobi 在我们的星巴克问题(61 城市)上**< 1 秒**就解出来——这背后就是 SEC + 懒约束的威力。
...
**直观含义**:每个城市**进**一次、**出**一次。如果你访问 5 个城市,每个城市来一次、走一次——很合理。

### 目标函数
$$\min \sum_i \sum_{j \ne i} d_{ij} x_{ij}$$

**就这么"看起来"对**——直觉上,每个城市进出各一次,最小化总距离,似乎就够了。
...
- 3 ↔ 4 互相来回走(另一个小环)。
- 度约束:每个节点入度 = 出度 = 1 ✓ 满足。
- 但**没有人从 {1,2} 走到 {3,4}**——这不是 TSP 要的"一条覆盖所有城市的环"。

这就叫**子环路 (subtour)**——一个**真子集**上的环。我们需要专门的约束来禁止。
...
**为什么 $|S| - 1$?**:
- 如果 $S$ 内部形成一个环,环长 = $|S|$ 条边($|S|$ 个城市的循环 = $|S|$ 条边)。
- 限制内部边数 ≤ $|S| - 1$ → **断开**这个潜在的环 → 至少有一条边必须"出去"。
- 这就强制 $S$ 和 $V \setminus S$ 之间有连接 → 整个图连通 → 行程完整。
...
**这个算法的"赌博":** 虽然理论上 SEC 有指数多个,实际上**用到的很少**——星巴克问题 61 个城市,只用了 91 条 SEC 就解出来。这是因为大部分子环路根本不会出现在最优解附近。

**懒约束的精髓**:**只在需要时加约束**。Gurobi 找到一个候选解 → 检查 → 如果有子环路,加几条 SEC → 继续。每加 1 条 SEC,只是排除"这一个"子环路。
...
**逐行解读:**
- `m.params.LazyConstraints = 1`:告诉 Gurobi"我会用懒约束"。**必须**设置,否则 cbLazy 调用无效。
- `def eliminateSubtours(model, where)`:回调函数。Gurobi 在不同时点调用它,`where` 标识时点。
- `where == GRB.Callback.MIPSOL`:**只在找到整数解时**触发。LP 节点(分数解)不触发——避免无用工作。
- `cbGetSolution(x)`:取当前候选整数解。
- `getSubtours`:把活跃边集分解成子环路列表。
- `if len(subtour_list) > 1`:**> 1 才是有子环路**。= 1 说明只有一个完整环,就是合法解,不加约束。
- `cbLazy(...)`:动态添加 SEC,排除当前子环路。
...
1. 准备一个未访问列表(所有节点)。
2. 当还有未访问的:
   a. 拿出未访问的第一个,作为新子环路的起点。
   b. 跟着 sequence 里"从这个节点出发的边"走到下一个节点。
   c. 如果下一个还没访问,加入当前子环路,继续走。
   d. 如果下一个已经访问过(回到起点了),当前子环路完成,跳出。
3. 把找到的所有子环路返回。

**关键技巧**:不需要知道整个图——**只看 sequence 里有的边**。每个节点恰好有一条出边(度约束),所以"下一步"唯一确定。
...
**手动追踪的步骤(考试常考):**
1. 在 sequence 里找以当前节点为起点的边,记录终点。
2. 把终点设为新当前,继续找它的出边。
3. 重复直到回到子环路的起点(或重复访问)。
4. 该子环路完成,选未访问中下一个节点开始新子环路。

**练习自己跑一遍**:从 0 出发,sequence 里 `(0,1)` → 到 1;`(1,8)` → 到 8;`(8,0)` → 到 0,回到起点了 → 子环路 [0,1,8] 完成。
...
**注意"不对称"**:Google Maps 给的时间 A→B 和 B→A 不一定相同——可能有单行道、立交桥、施工。所以 $d_{ij} \ne d_{ji}$ 一般成立,这是**有向 TSP**。SEC 里要同时包括两个方向。
...
**变量数:** $61 \times 60 = 3,660$ 个二元变量。SEC 约束如果全加:$2^{61} \approx 10^{18}$,显然不可能。所以这里**先不加**,靠回调动态加。
...
**惊人的数字:** 61 城市 TSP,< 1 秒解出,只用 ~101 条 SEC——而理论上有 $10^{18}$ 条可能的 SEC。这就是懒约束的真正威力——**实际需要的约束数比理论数小几个数量级**。
...
`> 0.5` 是从浮点数 0/1 提取二元解的标准技巧——避免精度误差。
...
**关键观察**:加 SEC 后**成本上升**了(从 36 到 43)——这是因为"完整行程"比"两个分散小环"更受约束,被迫绕路。SEC 不会让 TSP 解更便宜——它只是确保解是**合法**的。
...
**为什么仓库用曼哈顿距离?** 因为仓库通道是**网格**——只能沿走道走,不能斜穿货架。所以"水平 + 垂直距离"才是真实距离。城市街区也是同理(只能沿街道)。

**关键判断**:**对称** vs 非对称 → 决定后面 SEC 是否要同时写两个方向。曼哈顿/欧几里得通常对称;Google Maps 时间通常非对称。
...
**这个 trick 把"开放路径"变成"闭合行程"**——加一个虚拟节点 $n+1$,设它到起点的距离为 0,这样形式上还是 TSP,但实际路径是 起点 → ... → $n+1$(真终点)。
...
**big-M trick**:
- 如果 $x_{ij} = 1$(确实从 $i$ 去 $j$):约束变为 $t_j \ge t_i + d_{ij}$——"到 $j$ 的时间 ≥ 到 $i$ 时间 + 路上时间"。
- 如果 $x_{ij} = 0$:约束变为 $t_j \ge t_i + d_{ij} - M$,$M$ 很大,约束自动满足(无效)。

这是常见的**条件激活**技巧:用 big-M 让约束"只在某条件下生效"。
...
**TSP 变体的共同思想**:核心 TSP 公式不变,**加一组新变量 + 一组新约束**。考试可能给你一个变体场景,要你写完整 IP——掌握"加什么变量、加什么约束"是关键。
...
**怎么知道是 12 项?** $|S| = 4$ 时,内部有序对 = $4 \times 3 = 12$。
...
2. **有向 vs 无向混淆。** SEC 中包括 $x_{ij}$ **和** $x_{ji}$ 两个方向。
   - 即使是对称 TSP,$x_{ij}$ 和 $x_{ji}$ 是独立变量,SEC 里都要加。

3. **SEC 右端项写错。** 是 $|S| - 1$,**不是** $|S|$。
   - 直觉:$|S|$ 内部边数 = $|S|$ 才能构成环;$|S| - 1$ 断开了环。

4. **遗漏 $j \ne i$ 限定。** 用于排除自环。

5. **追踪 getSubtours 出错** — 仔细维护 unvisited 列表。
   - 错最多的:**忘记把当前节点从 unvisited 移除**。
...
8. **单节点的"子环路"** 不应生成 SECs(回调检查 $> 1$)。
   - len = 1 说明已经是完整解,不需要加约束。

9. **TSPTW 中 Big-M 太小。**
   - $M$ 必须 ≥ 最大可能时间,否则约束会被错误激活。
...
**类型 2 必考**(教授明确说了)——getSubtours 追踪。提前练熟。
...
总共 9 个节点(8 门店 + 1 仓库),变量 $9 \times 8 = 72$ 个。SEC 数 $2^9 - 2 = 510$——可以全部加,也可以用懒约束。

**Q2.** 位置 (20,30) 和 (60,50),速度 5 m/s。曼哈顿时间?

$d = |40| + |20| = 60$。$t = 60/5 = 12$s。

**注意**:这里是单边距离 / 时间。如果题目问"往返",要乘 2。
...
**手动追踪要点:** 维护 unvisited;每步把当前节点 remove;下一步从 sequence 找以当前节点为起点的边。
...
**对比 LP 松弛**:即使 SEC 数指数级,LP 松弛本身也无法暴力枚举所有 SEC。所以"现代 TSP 求解"就是"聪明地、按需地加 SEC"。
...
**Gurobi 的工作模式**:每个 MIPSOL 候选解检查一次。如果发现 3 个子环路,3 条 SEC 都加上,然后让 Gurobi 继续——下一次 MIPSOL 可能子环路变 1 或 2 个,继续加。直到 = 1(完整行程)。
...
**应对策略**:不要"全押一周"——准备**所有 8 周**,但每周抓核心 1-2 个题型。如果只准备某 4 周,可能正好这 4 周没考。
...
**最后的复习优先级**(给最终冲刺用):
1. **第 1 周 (LP 对偶)** — 几乎必考,因为它是后面所有 LP 题的基础。
2. **第 4 周 (选址 + 公式强度)**、**第 6 周 (定价 IP)**、**第 7 周 (网络流)** — 三大建模题。
3. **第 8 周 (TSP getSubtours)** — 已被教授**明确**确认会考。
4. **第 3 周 (报童)** — 一个公式 $F^{-1}((p-c)/p)$ 走天下,容易拿分。
5. **第 2 周、第 5 周** — 概念题(机会成本、蚕食),投入时间最少,只要懂概念就行。
